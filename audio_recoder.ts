...
interface QWebChannelInstance {
    objects: {
        shigeAudioRecorder: ShigeAudioRecBridge;
    };
}

declare var QWebChannel: {
    new(
        transport: unknown,
...
interface Window {
    isShigeAudioRec?: boolean;
    shigeAudioRecorder?: ShigeRecorder;
    AudioContext?: typeof AudioContext;
}

interface ShigeMediaSource {
    url: string;
    mimeType: string;
}

interface ShigeRecorder {
    bridge: ShigeAudioRecBridge | null;
    mediaStream: MediaStream | null;
    audioContext: AudioContext | null;
    sourceNode: MediaStreamAudioSourceNode | null;
    processorNode: ScriptProcessorNode | null;
    silentGainNode: GainNode | null;
    monitoredElement: HTMLMediaElement | null;
    strSessionId: string | null;
    isSessionActive: boolean;
    isStartPending: boolean;
    monitorRetryTimerId: number | null;
    sampleRate: number;
    channelCount: number;
    processorBufferSize: number;
    preRollChunkLimit: number;
    preRollChunks: string[];
    ...
    findActive(): HTMLMediaElement | null;
    findPrepared(): HTMLMediaElement | null;
    getStream(
    ...
    findDownloadSource(
        element: HTMLMediaElement | null
    ): ShigeMediaSource | null;
    requestAudioFileDownload(
        source: ShigeMediaSource
    ): boolean;
    makePcmType(): string;
    bufToBase64(
        buffer: ArrayBuffer
    ...
    clampSampleToInt16(sample: number): number;
...
(function () {
    ...
    function setupRecorder(): void {
        ...
        const recorder: ShigeRecorder = {
            ...
            //MARK:findActive
            findActive() {
                const allMediaElements = document.querySelectorAll<HTMLMediaElement>(
                    'audio, video'
                );

                let foundActiveElement: HTMLMediaElement | null = null;
            ...
            findPrepared() {
                ...
                let firstUsableElement: HTMLMediaElement | null = null;
            ...
            getStream(element) {
                ...
                const elementAsAnyType = element as HTMLMediaElement & {
            ...
            findDownloadSource(element) {
                ...
                const candidates: ShigeMediaSource[] = [];
                const seenUrls: { [key: string]: boolean } = {};

                const addCandidate = (url: string | null, mimeType: string | null): void => {
                    const normalizedUrl = (url || '').trim();
                ...
                const sourceElements = element.querySelectorAll<HTMLSourceElement>('source');
