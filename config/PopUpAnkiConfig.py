...
from aqt.utils import openLink, tooltip, showInfo, tr
...
class SetPopupConfig(QDialog):
    ...
    def __init__(self, parent=None):
        ...
        self.enable_browser_cookies = config.get("enable_browser_cookies", False)
        if self.enable_browser_cookies == "FirstRun":
        ...
        # Browser options
        self.enable_browser_cookies_label = self.create_checkbox(
            "Save Cookies (to disable it will delete all cookies)", "enable_browser_cookies")
        self.enable_adblocker_label = self.create_checkbox(
            "Enable Ad Blocker", "enable_adblocker")
        self.enable_auto_dl_audio_label = self.create_checkbox(
            "Enable Auto Download Audio", "enable_auto_dl_audio")
        ...
        # Browser Options tab =================================================
        other02_layout = QVBoxLayout()

        other02_layout.addWidget(QLabel(f"<b>[ Website ]</b>"))

        other02_layout.addWidget(self.enable_browser_cookies_label)


        other02_layout.addWidget(self.create_separator())#----------------
        other02_layout.addWidget(QLabel(f"<b>[ AdBlocker ]</b>"))
        other02_layout.addWidget(self.enable_adblocker_label)

        other02_layout.addWidget(self.create_separator())#----------------

        other02_layout.addWidget(QLabel(f"<b>[ Auto Save Audio ]</b>"))
        other02_layout.addWidget(self.enable_auto_dl_audio_label)

        qt_label_auto_save_audio = QLabel()
        text_auto_save_audio = """Auto saves audio played on websites or AI generated TTS and adds it to your cards. Some websites use special methods for audio playback, in such cases the add-on cannot save the audio. Saving audio may be prohibited on some websites due to copyright protection or their terms of service, these vary by website and country. Even if downloads are permitted redistribution is often prohibited so please be careful not to share the deck’s audio in such cases. Please check whether downloading is permitted or not and use this feature at your own risk, if you have concerns I recommend not using this feature. If you're looking for redistributable audio or a free batch generate solution I recommend my <a href="https://ankiweb.net/shared/info/428593773">add-on PyperTTS (https://ankiweb.net/shared/info/428593773</a>.
        """
        qt_label_auto_save_audio.setText(text_auto_save_audio)
        qt_label_auto_save_audio.setWordWrap(True)
        qt_label_auto_save_audio.setOpenExternalLinks(True)
        qt_label_auto_save_audio.setTextInteractionFlags(Qt.TextInteractionFlag.TextSelectableByMouse)
        qt_label_auto_save_audio.setFrameShadow(QFrame.Shadow.Sunken)
        qt_label_auto_save_audio.setStyleSheet("""
            border: 1px solid gray;
            border-radius: 5px;
            padding: 8px;
        """)
        other02_layout.addWidget(qt_label_auto_save_audio)


        other02_layout.addStretch(1)

        tab_browser = CustomWidget()
        tab_browser.setLayout(other02_layout)
        # tab ｸﾚｼﾞｯﾄ =================================================
        ...
        tab_widget.addTab(tab_browser, "Other2")
    ...
    def handle_button_clicked(self):
        ...
        try:
            ...
            from ..dock_web_view import dock_content, close_all_dock_widget
            ...
            QTimer.singleShot(1000, ChatGPT_URL_open)
    ...
    def save_config_fontfamiles(self):
        ...
        # browser options
        config["enable_browser_cookies"] = self.enable_browser_cookies
        config["enable_adblocker"] = self.enable_adblocker
        config["enable_auto_dl_audio"] = self.enable_auto_dl_audio
